...
vector<int> Modified_BFS(const Bipartite_Graph& G, const Bipartite_Graph& M) {
    ...
    // A szomszédsági listákat közvetlenül lekérjük.
    // Így nem használjuk a Neighbours_Of_A / Neighbours_Of_B függvényeket.
    ...
    // A_a akkor szabad, ha M-ben nincs szomszedja.
    for (int a = 1; a <= n_A; a++) {
        if (M_A[a - 1].empty()) {
            tav_A[a] = 0;
    ...
    while (eleje < L.size()) {
        ...
        if (Is_A_Vertex(aktiv_csucs)) {
            ...
            // A-bol B-be a G graf elein megyunk,
            // de csak olyan eleken, amelyek nincsenek benne M-ben.
            for (int b : G_A[a - 1]) {
            ...
            // B-bol A-ba csak az M parositasbeli elen megyunk.
            for (int a : M_B[b - 1]) {
                if (tav_A[a] == INF) {
                    tav_A[a] = tav_B[b] + 1;
...
void Improve_Matching(Bipartite_Graph& M, const vector<int>& P) {
    ...
    for (int i = 0; i + 1 < P.size(); i++) {
        ...
        if (first > 0 && second < 0) {
            ...
            // Hibás él, elvileg nem fordulhat elő
            continue;
        }

        if (Is_Matching_Edge(M, a, b)) {
            M.DeleteEdge(a, b);
        }
        else {
            M.AddEdge(a, b);
